...
public interface IPZEnergyHandler extends ISidedEnergyHandler {
    ...
    @Override
    default void setEnergy(int storage, int energy, @Nullable Direction side) {
    ...
    @Override
    default int getEnergy(int storage, @Nullable Direction side) {
        return getEnergyContainer(storage, side).map(IEnergyContainer::getEnergy).orElse(0);
    }

    @Override
    default int getEnergyCapacity(int storage, @Nullable Direction side) {
        return getEnergyContainer(storage, side).map(IEnergyContainer::getCapacity).orElse(0);
    }

    @Override
    default boolean canInsert(int storage, @Nullable Direction side) {
        return getEnergyContainer(storage, side).map(c -> c.canInsert(AutomationType.handler(side))).orElse(false);
    }

    @Override
    default boolean canExtract(int storage, @Nullable Direction side) {
        return getEnergyContainer(storage, side).map(c -> c.canExtract(AutomationType.handler(side))).orElse(false);
    }
}
